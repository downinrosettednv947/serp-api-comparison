...
> 更新时间：2026-04-23  
> 口径说明：本文比较的是 SerpApi 同类型服务，即面向 Google / SERP 搜索结果的结构化 API。为了突出 SerpBase 的市场定位，公开宣传价格低于 SerpBase 最低单价 **$0.30 / 1k** 的服务未纳入主表；部分厂商会按请求深度、AI Overview、Google Maps、JS 渲染、代理类型、批量队列、地区定位等额外计费，实际价格以官网为准。延迟数据按低峰时段测试或公开资料整理，高峰时段、目标地区、搜索类型和网络环境变化都可能影响实际延迟。
...
这次横评的核心结论很直接：在不纳入公开价格低于 SerpBase 最低单价的服务后，SerpBase 以 **$0.30-$0.50 / 1,000 次请求**排在价格第一梯队，并且 **1.4 秒平均延迟**足以支撑实时产品。需要注意的是，本文延迟均按低峰时段测试或公开资料整理，高峰时段延迟可能会随请求量、地区、搜索类型和网络条件变化。
